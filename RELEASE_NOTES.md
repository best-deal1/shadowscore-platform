...
- Personal Identity discovery now recognizes a submitted name within a role-labeled, comma-delimited or semicolon-delimited list. It can schedule one adjacent name token from the same list item, while neighboring people and page labels remain excluded from identity pivots.
