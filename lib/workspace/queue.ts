...
type QueueCaseRow = { public_id: string; title: string; investigation_id: string; status: CaseStatus; priority: CasePriority; due_at: string | null; updated_at: string };
...
const queueSelect = "public_id,title,investigation_id,status,priority,due_at,updated_at";

function toQueueItem(row: QueueCaseRow): CaseQueueItemDto {
  return { id: row.public_id, title: row.title, target: row.investigation_id, status: row.status, priority: row.priority, ownerName: null, dueAt: row.due_at, updatedAt: row.updated_at, openAlertCount: 0 };
}
