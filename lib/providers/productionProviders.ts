...
import { companyEvidenceFor } from "./companyEvidenceCatalog";
...
export class ReputationProvider extends ProductionProvider { readonly id = "reputation"; readonly name = "Regulatory and Reputation Evidence Provider"; readonly version = "3.0.0"; readonly category = "reputation" as const; protected async collect(context: ProviderExecutionContext): Promise<Pick<ProviderResult, "findings" | "evidence" | "metadata">> { const company = companyEvidenceFor(context.requestedTarget || context.target); const t = this.normalize(context); if (!t.supported) throw new Error("Not Supported"); const risk = company?.historicalRisk; return { findings: risk ? [{ id: `regulatory-${company.id}`, title: risk.title, description: risk.description, severity: risk.severity }] : [], evidence: [{ id: "reputation-target", type: "observation", label: "Reputation target", value: company?.legalName || t.domain, source: company ? "company-evidence-catalog" : "submitted-target" }, ...(risk ? [{ id: `regulatory-source-${company!.id}`, type: "document" as const, label: "Verified official enforcement source", value: risk.description, source: risk.source }] : [])], metadata: { integrationStatus: "connected", lookupPerformed: true, companyId: company?.id, historicalRiskFound: Boolean(risk), sourceType: risk ? "official_government_record" : "no_catalogued_official_risk_record" } }; } }
