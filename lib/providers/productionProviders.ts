...
function decodeHtml(value: string | undefined) { return value?.replace(/&amp;/g, "&").replace(/&#39;/g, "'").replace(/&quot;/g, '"').replace(/\s+/g, " ").trim(); }
function firstMeta(html: string, property: string) { return decodeHtml(html.match(new RegExp(`<meta[^>]+(?:property|name)=["']${property}["'][^>]+content=["']([^"']+)`, "i"))?.[1]); }
function cleanOrgName(value: string | undefined) { const cleaned = decodeHtml(value)?.replace(/\s*[|–—-]\s*(Official Site|Home|Homepage|Global|United States|USA|US|Payments?|Internet|Commerce|Ecommerce).*$/i, "").trim(); return cleaned && cleaned.length <= 120 ? cleaned : undefined; }
function schemaOrganizationNames(html: string) { const names: string[] = []; for (const match of html.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) { try { const parsed = JSON.parse(match[1].replace(/&quot;/g, '"')); const queue = Array.isArray(parsed) ? [...parsed] : [parsed]; while (queue.length) { const item = queue.shift(); if (!item || typeof item !== "object") continue; if (Array.isArray(item)) { queue.push(...item); continue; } const type = Array.isArray(item["@type"]) ? item["@type"].join(" ") : String(item["@type"] || ""); if (/Organization|Corporation|LocalBusiness|OnlineBusiness|Store/i.test(type) && typeof item.name === "string") names.push(item.name); if (item["@graph"]) queue.push(item["@graph"]); } } catch {} } return Array.from(new Set(names.map(cleanOrgName).filter((name): name is string => Boolean(name)))); }
function copyrightOwner(html: string) { return cleanOrgName(html.match(/(?:©|&copy;|Copyright)\s*(?:\d{4})?\s*([^<\n|]+?)(?:\.\s*All rights reserved|All rights reserved|<|$)/i)?.[1]); }

export class BusinessProfileProvider extends ProductionProvider { readonly id = "business-profile"; readonly name = "Public Business Profile Provider"; readonly version = "2.1.0"; readonly category = "business_profile" as const; protected async collect(context: ProviderExecutionContext): Promise<Pick<ProviderResult, "findings" | "evidence" | "metadata">> { const t = this.normalize(context); if (!t.supported || !t.websiteUrl) throw new Error("Not Supported"); const r = await acquireTimedSharedHttp(context); const blocked = isErrorPage(r); const html = blocked || r.diagnostics.responseWasHtml === false ? "" : r.text; const title = cleanOrgName(html.match(/<title[^>]*>([^<]+)<\/title>/i)?.[1]); const schemaNames = schemaOrganizationNames(html); const siteName = cleanOrgName(firstMeta(html, "og:site_name") || firstMeta(html, "application-name")); const owner = copyrightOwner(html); const org = schemaNames[0] || siteName || title; const finalUrl = r.finalUrl || r.requestedUrl; const evidence: ProviderEvidence[] = [{ id: "profile-domain", type: "observation", label: "Business website domain", value: t.domain, source: finalUrl }, { id: "profile-title", type: "document", label: "Business profile title", value: title || "unavailable", source: finalUrl }, { id: "profile-organization", type: "document", label: "Business name", value: org || "unavailable", source: finalUrl }, { id: "schema-organization-name", type: "document", label: "Schema.org organization name", value: schemaNames[0] || "unavailable", source: `${finalUrl}#schema.org` }, { id: "metadata-site-name", type: "document", label: "Website site name", value: siteName || "unavailable", source: `${finalUrl}#metadata` }, { id: "footer-copyright-owner", type: "document", label: "Copyright owner", value: owner || "unavailable", source: `${finalUrl}#footer` }]; const outcome = !r.ok ? r.outcome : (blocked ? r.outcome : (org || title || schemaNames.length || owner ? "completed_with_evidence" : "completed_no_extractable_evidence")); return { findings: [], evidence, metadata: { integrationStatus: "connected", lookupPerformed: true, domain: t.domain, businessName: org, countrySupport: "global_public_web", extractedIdentityFields: { legalNames: [], schemaOrganizationNames: schemaNames, pageTitle: title, copyrightOwner: owner, privacyPolicyEntities: [], termsEntities: [] }, httpOutcome: outcome, errorPageTitleRejected: blocked && Boolean(title), httpDiagnostics: r.diagnostics, httpAttempts: r.attempts, sharedHttpFetchCount: (context as MutableProviderContext).sharedHttpFetchCount } }; } }
