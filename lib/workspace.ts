...
export type ScanMode = "website" | "marketplace" | "evidence" | "identity";
...
export type ShadowScoreIntake = {
  ...
  caseType?: string;
  email: string;
  identitySignals?: import("./identity/investigation").PersonalIdentitySignals;
  fileNames: string[];
...
export async function getWorkspace(session: WorkspaceSession): Promise<WorkspaceData> {
  if (isSupabaseConfigured() && session.accessToken) {
    ...
    return presentWorkspaceForEndUser({
      ...
      intakes: intakeRows.map((row) => ({ intakeId: row.intake_id, userId: row.user_id, scanMode: row.scan_mode, target: row.target, platform: row.platform, caseType: row.case_type, email: row.email, identitySignals: row.identity_signals || undefined, fileNames: row.file_names || [], visibleSignalCategories: row.visible_signal_categories || [], paymentStatus: row.payment_status, reportStatus: row.report_status, createdAt: row.created_at })),
  ...
  if (isSupabaseConfigured() && session.accessToken) {
    const [created] = await supabaseFetch<Record<string, any>[]>("/rest/v1/intakes?select=*", {
      ...
      body: JSON.stringify({
        ...
        email: intake.email,
        ...(intake.scanMode === "identity" ? { identity_signals: intake.identitySignals } : {}),
