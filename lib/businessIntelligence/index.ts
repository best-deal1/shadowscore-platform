...
type Observation = BusinessEvidenceReference & { normalizedValue: string; field: Field };
...
function add(out: Observation[], result: ProviderResult, evidence: Pick<ProviderEvidence, "id" | "label" | "value" | "source">, key?: string) {
  ...
  if (!field) return;
  out.push({ ...evidence, value: evidence.value.trim(), source: evidence.source || result.providerId, providerId: result.providerId, observedAt: result.completedAt, field, normalizedValue: normalize(evidence.value) });
}
...
export function buildBusinessIntelligence(providerResults: ProviderResult[], generatedAt = new Date().toISOString()): BusinessIntelligenceResult {
  ...
  for (const field of ["identity", "ownership", "payment", "operations", "claim"] as Field[]) {
    const items = observed.filter((item) => item.field === field);
