...
type EvidenceSourceRow={source_id:string};
export async function executeClaimedWebsiteJob(baseUrl:string,serviceKey:string,job:ClaimedJob){
 ...
 // Wave 2 dual-write. The legacy observations and report remain authoritative until parity is verified.
 const sources=await call("/rest/v1/evidence_sources?source_key=eq.website-intelligence&schema_version=eq.1&select=source_id&limit=1") as EvidenceSourceRow[];
 if(sources[0]){
  const instances=await call("/rest/v1/evidence_source_instances",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({source_id:sources[0].source_id,investigation_id:job.investigation_job_id,external_reference:scanId,acquired_at:report.scannedAt,effective_at:report.scannedAt,payload_reference:`website_intelligence_scans:${scanId}`,payload_hash:scanId,collector_key:"website_intelligence",collector_version:"website-intelligence-v1",metadata:{legacyAuthoritative:true}})}) as Array<{source_instance_id:string}>;
  const v2=report.modules.flatMap(module=>module.evidence.map(item=>({source_instance_id:instances[0].source_instance_id,subject_id:job.subject_id,investigation_id:job.investigation_job_id,observation_type:`website.${module.moduleId}`,observation_key:item.id,value:{label:item.label,value:item.value,source:item.source},observed_at:item.observedAt,valid_from:item.observedAt,metadata:{scanId,moduleStatus:module.status,confidence:module.confidence}})));
  if(v2.length)await call("/rest/v1/evidence_v2_observations",{method:"POST",body:JSON.stringify(v2)});
 }
