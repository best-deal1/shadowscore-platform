...
const { resolveBusinessIdentity } = require("../businessIdentityResolver.js");
...
const authoritativeFallback = new Map([
  ["MSFT", [789019, "MICROSOFT CORP", "MSFT", "Nasdaq"]], ["AAPL", [320193, "Apple Inc.", "AAPL", "Nasdaq"]], ["AMZN", [1018724, "AMAZON COM INC", "AMZN", "Nasdaq"]], ["NET", [1477333, "Cloudflare, Inc.", "NET", "NYSE"]], ["0000320193", [320193, "Apple Inc.", "AAPL", "Nasdaq"]],
]);
function fallbackResult(entry) { const [cikNumber, legalName, ticker, exchange] = authoritativeFallback.get(entry.target); const cik = String(cikNumber).padStart(10, "0"); const sourceUrl = "https://www.sec.gov/files/company_tickers_exchange.json"; const evidence = ["legal-name", "ticker", "exchange", "cik"].map((part)=>({ id: `sec-${ticker.toLowerCase()}-${part}`, type: "document", label: `SEC ${part}`, value: part === "legal-name" ? legalName : part === "ticker" ? ticker : part === "exchange" ? exchange : cik, source: sourceUrl })); return { providerId: "authoritative-company", providerVersion: "1.0.0", status: "completed", startedAt: new Date().toISOString(), completedAt: new Date().toISOString(), duration: 0, findings: [], evidence, metadata: { legalName, ticker, exchange, cik, sourceUrl, resolverEvidence: { id: `sec:${cik}`, legalName, ticker, exchange, verified: true, verificationStatus: "authoritative", source: "sec_company_tickers_exchange_recorded_fixture", evidenceRefs: evidence.map((i)=>i.id), observedAt: "2026-01-01T00:00:00.000Z" } }, errors: ["Live SEC fetch unavailable; used recorded SEC fixture for deterministic validation."] }; }
