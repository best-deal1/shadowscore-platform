...
const STRONG_IDENTIFIERS = new Set<InvestigationInputKind>(["email", "phone", "registration_number", "domain", "marketplace_identity"]);
...
function resolveEntities(candidates: EntityCandidate[], evidence: EvidenceAssertion[]) {
  ...
  const entities: ResolvedEntity[] = [...grouped.values()].map((group) => {
    ...
    const sourceCount = new Set(evidence.filter((item) => evidenceIds.includes(item.evidenceId)).map((item) => item.source.sourceId)).size;
...
function detectContradictions(evidence: EvidenceAssertion[], entityByCandidate: Map<string, ResolvedEntity>) {
  ...
  for (const item of evidence.filter((entry) => CONFLICTING_RELATIONSHIPS.has(entry.relationship))) {
  ...
  for (const [key, items] of groups) {
    ...
    const sources = new Set(items.map((item) => item.source.sourceId));
  ...
  for (const entity of new Set(entityByCandidate.values())) for (const identifier of entity.identifiers.filter((item) => ["email", "phone", "domain"].includes(item.kind))) {
    const key = normalize(identifier);
...
export function buildInvestigationGraph(input: InvestigationEngineInput): InvestigationGraph {
  ...
  const average = evidence.length ? Math.round(evidence.reduce((sum, item) => sum + item.confidence, 0) / evidence.length) : 0;
  const decision = critical ? { outcome: "stop" as const, confidence: average, summary: "Critical contradictions require the transaction to stop pending resolution.", nextActions: ["Resolve the critical evidence conflicts with primary-source records."] } : high ? { outcome: "investigate" as const, confidence: average, summary: "Material identity conflicts require further investigation.", nextActions: ["Verify reused identifiers and ownership against independent primary sources."] } : average >= 75 ? { outcome: "proceed" as const, confidence: average, summary: "Independent evidence supports the resolved entity and its relationships.", nextActions: ["Retain the evidence trail with the customer decision."] } : { outcome: "proceed_with_conditions" as const, confidence: average, summary: "The entity graph is incomplete or supported by limited evidence.", nextActions: ["Collect another independent source for unresolved relationships."] };
