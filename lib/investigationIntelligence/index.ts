...
export function buildInvestigationIntelligence(input: InvestigationIntelligenceInput): InvestigationIntelligence {
  ...
  const negativeEvidence = evidenceItems.filter((item) => item.category === "Negative");
  const risks = [
  ...
  const gaps = evidenceItems.filter((item) => ["Missing", "Unavailable", "Not Checked"].includes(item.category));
