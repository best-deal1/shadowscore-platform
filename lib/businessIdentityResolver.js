const BUSINESS_IDENTITY_RESOLVER_VERSION = "canonical-identity-resolver-v2";
...
function resolveBusinessIdentity(input, options = {}) {
  ...
  for (const item of [...(options.seeds || []), options.targetClassifier, options.businessProfile, ...(options.providerEvidence || []), options.websiteMetadata, ...(options.marketplaceEvidence || []), ...(options.relationshipEvidence || [])].filter(Boolean)) ingestEvidence(item, graph, options);
