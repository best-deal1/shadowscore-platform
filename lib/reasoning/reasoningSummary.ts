...
export function summarizeReasoning(input: Input): ReasoningSummary {
  ...
  return {
    keyConclusions: positive.slice(0, 5).map((step) => `${step.inferredFact} (${step.confidence >= 85 ? "Very High" : step.confidence >= 65 ? "High" : step.confidence >= 40 ? "Medium" : "Low"} inferred confidence).`),
    ...
    confidencePropagation: input.steps.map((step) => `${step.id}: evidence confidence ${step.supportingEvidence.length ? "available" : "none"}, assumptions ${step.assumptions.length}, propagated ${step.confidence >= 85 ? "Very High" : step.confidence >= 65 ? "High" : step.confidence >= 40 ? "Medium" : "Low"}.`),
  };
}
