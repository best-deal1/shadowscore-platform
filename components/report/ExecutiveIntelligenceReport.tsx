...
export default function ExecutiveIntelligenceReport({ report }: { report: ShadowScoreReport }) {
  ...
  return <article className="mt-8 overflow-hidden rounded-[32px] bg-slate-100 text-slate-700 shadow-2xl shadow-black/30">
    ...
    <section className="border-b border-slate-200 bg-white p-5 sm:p-8" aria-labelledby="executive-recommendation">
      <div className={`rounded-3xl border p-6 sm:p-8 ${tone}`}>
        <p className="text-xs font-bold uppercase tracking-[0.2em] opacity-70">Executive Recommendation</p>
        <div className="mt-3 flex flex-wrap items-center justify-between gap-5"><div><h2 id="executive-recommendation" className="text-3xl font-black sm:text-4xl">{recommendation.label}</h2><p className="mt-3 max-w-3xl leading-7 opacity-80">{recommendation.explanation}</p></div><div className="rounded-2xl bg-white/70 px-5 py-4 text-center"><p className="text-xs font-bold uppercase tracking-wider opacity-60">Trust score</p><p className="mt-1 text-3xl font-black">{trustScore}</p><p className="mt-1 text-xs opacity-60">Confidence {confidence}</p></div></div>
    ...
    <div className="min-h-[420px] p-5 sm:p-8">
      {activeTab === "Summary" && <section><h2 className="text-2xl font-bold text-slate-950">Executive Summary</h2><p className="mt-2 max-w-4xl leading-7 text-slate-600">{report.reportSummary?.message || recommendation.explanation}</p><div className="mt-6 grid gap-4 sm:grid-cols-3"><ReportMetric label="Risk Score Card" value={trustScore} detail="Trust score out of 100" /><ReportMetric label="Evidence Summary" value={`${evidenceGroups.reduce((total, group) => total + group.items.length, 0)}`} detail="Distinct evidence records" /><ReportMetric label="Source Appendix" value={`${sources.length}`} detail="Sources documented" /></div><div className="mt-4 grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border border-slate-200 bg-white p-6"><h3 className="font-bold text-slate-950">Key Findings</h3><ul className="mt-4 space-y-3">{[...findings.positive, ...findings.warnings, ...findings.negative].slice(0, 3).map((item) => <li key={item.id} className="flex gap-3"><span aria-hidden="true">{findings.positive.includes(item) ? "✓" : "!"}</span><span>{item.title}</span></li>)}</ul>{findings.positive.length + findings.warnings.length + findings.negative.length === 0 && <p className="mt-4 text-sm text-slate-500">No material findings were recorded.</p>}</div><div className="rounded-2xl border border-slate-200 bg-white p-6"><h3 className="font-bold text-slate-950">Recommended Actions</h3><ol className="mt-4 space-y-3">{actions.slice(0, 3).map((action, index) => <li key={action} className="flex gap-3"><span className="font-bold text-cyan-700">{index + 1}</span><span>{action}</span></li>)}</ol></div></div><div className="mt-4 rounded-2xl border border-slate-200 bg-white p-6"><h3 className="font-bold text-slate-950">Investigation Timeline</h3><p className="mt-2 text-sm text-slate-500">{timeline.length} recorded event{timeline.length === 1 ? "" : "s"}. Open Timeline for the complete sequence.</p></div></section>}
...
function ReportMetric({ label, value, detail }: { label: string; value: string; detail: string }) {
  return <div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-xs font-bold uppercase tracking-wider text-slate-500">{label}</p><p className="mt-2 text-3xl font-black text-slate-950">{value}</p><p className="mt-1 text-xs text-slate-500">{detail}</p></div>;
}
