    ...
    <article className="executive-report overflow-hidden rounded-[28px] border border-slate-300 bg-[#f4f3ef] text-slate-700 shadow-[0_28px_90px_rgba(0,0,0,0.32)]">
      ...
      <div className="px-6 py-8 sm:px-10 lg:px-14 lg:py-12">
        ...
        <section id="decision-brief" aria-labelledby="summary-title" className="mt-12 border-t border-slate-300 pt-10">
          ...
          <div className="mt-6 border border-slate-300 bg-slate-50 p-5"><h3 className="text-xs font-bold uppercase tracking-[0.18em] text-slate-500">Investigation Timeline</h3><dl className="mt-4 grid grid-cols-2 gap-x-5 gap-y-4 sm:grid-cols-3 lg:grid-cols-6">{[["Evidence items recorded", execution?.evidenceCollected ?? evidence.length], ["Source checks completed", execution?.providersExecuted ?? sources.length], ["Evidence conflicts", intelligence?.contradictions.length ?? 0], ["Evidence relationships", intelligence?.relationships.length ?? graphSummary?.relationshipCount ?? 0], ["Decision confidence", confidence], ["Report preparation", execution ? `${execution.completedInSeconds}s` : "Not recorded"]].map(([label, value]) => <div key={label}><dt className="text-xs text-slate-500">{label}</dt><dd className="mt-1 text-lg font-semibold text-slate-950">{value}</dd></div>)}</dl></div>
        </section>
