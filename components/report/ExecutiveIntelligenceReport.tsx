...
function isPersonalReport(report: ShadowScoreReport) {
  return ["EMAIL", "PERSONAL_IDENTITY"].includes(report.reportSummary?.investigationType || "");
}
...
function normalizeIdentityCandidate(candidate: NonNullable<ShadowScoreReport["reportSummary"]>["publicIdentityCandidates"] extends (infer Item)[] | undefined ? Item : never, index: number) {
  const legacyAttribution = candidate.identityAttributionConfidence ?? candidate.confidence;
  return {
    ...candidate,
    ...
    verificationLabel: candidate.identityAttributionConfidence === null ? "Unverified" : "Resolver assessed",
    legacyAttribution,
...
function caseObjective(report: ShadowScoreReport) {
  if (["EMAIL", "PERSONAL_IDENTITY"].includes(report.reportSummary?.investigationType || "")) return "Investigate the submitted email as a person or identity identifier and assess public evidence without attributing the mailbox provider's infrastructure to the subject.";
...
const businessInvestigationScope = ["Business identity", "Ownership", "Digital presence", "Infrastructure", "Payment information", "Independent sources", "Evidence supplied by the customer"] as const;
const personalInvestigationScope = ["Submitted identity signals", "Public profile discovery", "Exact identifier matches", "Conflicting identity signals", "Independent sources", "Reference material"] as const;
...
export default function ExecutiveIntelligenceReport({ report }: { report: ShadowScoreReport }) {
  ...
  const gaps = isPersonalReport(report) ? report.reportSummary?.personalIdentityGaps || [] : Array.from(new Set(scorecard.flatMap((item) => item.evidenceGaps))).slice(0, 8);
  ...
  const isEmailInvestigation = ["EMAIL", "PERSONAL_IDENTITY"].includes(report.reportSummary?.investigationType || "");
  const investigationScope = isEmailInvestigation ? personalInvestigationScope : businessInvestigationScope;
    ...
    <article className="executive-report overflow-hidden rounded-[28px] border border-slate-300 bg-[#f4f3ef] text-slate-700 shadow-[0_28px_90px_rgba(0,0,0,0.32)]">
      ...
      <header className="bg-[#10263d] px-6 py-9 text-white sm:px-10 lg:px-14 lg:py-12">
        <div className="flex items-center justify-between gap-6 border-b border-white/20 pb-6">
          <div><p className="text-xs font-bold uppercase tracking-[0.3em] text-cyan-300">ShadowScore Intelligence</p><p className="mt-2 text-sm text-slate-300">{isEmailInvestigation ? "Personal identity evidence review" : "Independent business risk review"}</p></div>
        ...
        <div className="mt-9 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          <div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-300">Investigation Case File</p><h1 className="mt-3 max-w-3xl text-4xl font-semibold tracking-[-0.035em] sm:text-5xl">{narrative?.businessName || report.target || report.entity}</h1><p className="mt-4 max-w-2xl text-base leading-7 text-slate-300">{isEmailInvestigation ? "Person-specific report separating discovery leads from identity evidence." : "Commercial due diligence record prepared for executive review and decision support."}</p></div>
      ...
      <div className="px-6 py-8 sm:px-10 lg:px-14 lg:py-12">
        ...
        <section id="identity" aria-labelledby="identity-title" className="mt-12 border-t border-slate-300 pt-10"><p className="text-xs font-bold uppercase tracking-[0.22em] text-cyan-800">02 / Entity profile</p><h2 id="identity-title" className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">{isEmailInvestigation ? "Personal Identity" : "Business Identity"}</h2><dl className="mt-6 grid border-y border-slate-300 bg-white sm:grid-cols-2 lg:grid-cols-4">{[[isEmailInvestigation ? "Primary submitted signal" : "Legal or trading name", isEmailInvestigation ? report.target || report.entity : narrative?.businessName || report.entity], ["Reviewed target", report.target || report.entity], ["Investigation type", investigationType], ["Identity confidence", levelLabel(scorecard.find((item) => item.dimension === "Identity Confidence")?.level)]].map(([label, value]) => <div key={label} className="border-b border-slate-200 p-5 last:border-b-0 sm:border-r sm:[&:nth-child(2n)]:border-r-0 lg:border-b-0 lg:[&:nth-child(2n)]:border-r lg:last:border-r-0"><dt className="text-xs font-bold uppercase tracking-wider text-slate-500">{label}</dt><dd className="mt-2 font-semibold text-slate-950">{value}</dd></div>)}</dl>
          {isEmailInvestigation && report.reportSummary?.identitySignals && <div className="mt-6 border border-slate-300 bg-white p-5"><h3 className="font-semibold text-slate-950">User-submitted reference signals</h3><p className="mt-2 text-sm text-slate-600">These details guided discovery. They are not independently verified public evidence.</p><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">{Object.entries(report.reportSummary.identitySignals).map(([key, signal]) => signal && <div key={key}><dt className="font-semibold capitalize text-slate-950">{key.replace(/([A-Z])/g, " $1")}</dt><dd className="mt-1 break-all">{signal.value} <span className="text-slate-500">({signal.provenance})</span></dd></div>)}</dl>{report.reportSummary.imageComparisonStatus === "not_operational" && <p role="status" className="mt-4 border border-amber-300 bg-amber-50 p-3 text-sm text-amber-950">Automated image comparison is not operational. The reference image was not used to score candidates.</p>}</div>}
