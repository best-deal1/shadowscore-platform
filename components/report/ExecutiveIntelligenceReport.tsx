...
function normalizeIdentityCandidate(candidate: NonNullable<ShadowScoreReport["reportSummary"]>["publicIdentityCandidates"] extends (infer Item)[] | undefined ? Item : never, index: number) {
  return {
    ...
    resolutionRank: candidate.resolutionRank || index + 1,
    resolverMatchedSignals: candidate.resolverMatchedSignals || [],
    resolverConflictingSignals: candidate.resolverConflictingSignals || [],
...
export default function ExecutiveIntelligenceReport({ report }: { report: ShadowScoreReport }) {
  ...
  const gaps = Array.from(new Set([
    ...scorecard.flatMap((item) => item.evidenceGaps).filter((gap) => !isEmailInvestigation || !personalInfrastructure.test(gap)),
    ...(isEmailInvestigation ? ["Profile ownership has not been confirmed by the subject.", "A second independent person-specific identifier is still required."] : []),
  ])).slice(0, 8);
