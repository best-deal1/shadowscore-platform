...
function evidence(evidenceId, subjectCandidateId, relationship, value, sourceRef, options = {}) {
  return { evidenceId, subjectCandidateId, relationship, value, source: sourceRef, confidence: options.confidence ?? 90, evidenceType: options.evidenceType ?? "other", objectCandidateId: options.objectCandidateId };
}
...
test("resolves exact identifiers into one graph without merging on similar names", () => {
  const graph = buildInvestigationGraph({
    ...
    evidence: [
      evidence("registry", "company-registry", "registered_name", "Acme Trading Ltd", source("registry")),
      evidence("website", "company-site", "contact_email", "owner@acme.example", source("website")),
      evidence("directory", "similar-name", "directory_name", "Acme Trading Group", source("directory"), { confidence: 45 }),
    ],
  });
...
test("connects marketplace evidence to the resolved company and decision", () => {
  const graph = buildInvestigationGraph({
    ...
    now: "2026-08-09T00:00:00.000Z",
    candidates: [
      candidate("company", "company", "Acme Ltd", [{ kind: "registration_number", value: "123" }], ["reg", "market-link"]),
      candidate("seller", "marketplace_account", "Acme Store", [{ kind: "marketplace_identity", value: "shop/acme" }], ["market-link"]),
    ],
    evidence: [
      evidence("reg", "company", "registered_name", "Acme Ltd", source("registry"), { evidenceType: "registry" }),
      evidence("market-link", "seller", "operated_by", "Acme Ltd", source("marketplace"), { evidenceType: "marketplace", objectCandidateId: "company" }),
    ],
  });

  assert.equal(graph.marketplace.entityIds.length, 1);
  assert.deepEqual(graph.marketplace.evidenceIds, ["market-link"]);
...
test("surfaces independent ownership conflicts and lowers the decision", () => {
  ...
  const graph = buildInvestigationGraph({
    ...
    evidence: [
      evidence("whois", "domain", "domain_owner", "Acme Holdings Ltd", source("registry")),
      evidence("claim", "domain", "domain_owner", "Example Seller LLC", source("marketplace"), { evidenceType: "marketplace" }),
    ],
  });
