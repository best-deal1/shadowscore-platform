...
import { identityInvestigationGuardIssues, normalizeIntakeIdentitySignals } from "@/lib/personalIdentity";
...
export async function POST(request: Request) {
  try {
    ...
    const investigationRouting = resolveInvestigationRouting({ target: body.target, scanMode: body.scanMode });
    const personalIdentityInvestigation = investigationRouting.primaryInvestigationType === "PERSON_IDENTITY";
    const identitySignals = normalizeIntakeIdentitySignals(body.identitySignals, { target: investigationRouting.submittedSeed, email: body.email });
    const guardIssues = identityInvestigationGuardIssues(personalIdentityInvestigation, identitySignals, process.env);
    const readinessIssues = guardIssues.filter((issue) => issue !== "At least one identity signal is required.");
    ...
    if (guardIssues.length) return authenticatedResponse({ error: "Add at least one identity signal." }, 400, authenticated.refreshedAuth);
    ...
    const intake = await createIntake(session, {
      ...
      identitySignals: personalIdentityInvestigation ? identitySignals : undefined,
