...
export async function POST(request:Request){
  ...
  try{const result=await executeClaimedWebsiteJob(config.url,serviceKey,jobs[0] as Parameters<typeof executeClaimedWebsiteJob>[2]);return NextResponse.json({claimed:true,result});}
