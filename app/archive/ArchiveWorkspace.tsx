...
export function ArchiveWorkspace({ initialInvestigations, canManage }: { initialInvestigations: readonly CaseQueueItemDto[]; canManage: boolean }) {
  ...
  async function update(item: CaseQueueItemDto, action: "restore" | "delete") {
    ...
    try {
      ...
      setNotice(`${item.title} was ${action === "delete" ? "deleted" : "restored to Investigations"}.`);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "The investigation could not be updated. Try again.");
    } finally {
      setPendingId(null);
    }
  }
