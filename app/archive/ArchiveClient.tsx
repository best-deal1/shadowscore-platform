...
export default function ArchiveClient() {
  ...
  useEffect(() => { const session = getCurrentSession(); if (!session) { router.replace("/login?returnTo=%2Farchive"); return; } getWorkspace(session).then((data) => setReports(data.reports.filter((r) => r.reportStatus === "ready" && r.paymentStatus === "paid"))).catch(() => setError("Archive could not be loaded. Refresh the page or contact support.")).finally(() => setLoading(false)); }, [router]);
