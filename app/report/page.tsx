...
function ExecutiveBrief({ report }: { report: ShadowScoreReport }) {
  ...
  return (
    <article className="mt-8 rounded-[32px] border border-white/10 bg-black/55 p-6 shadow-[0_0_60px_rgba(120,0,20,0.16)] sm:p-8">
      ...
      {report.reportSummary?.canonicalWebsiteReport && <section className="mt-5 rounded-3xl border border-white/10 bg-black/35 p-6" aria-label="Website Intelligence"><WebsiteIntelligenceDashboard report={report.reportSummary.canonicalWebsiteReport} /><WebsiteChangeTimeline changeReport={report.reportSummary.websiteChangeReport} timeline={report.reportSummary.websiteChangeTimeline} /><div className="my-10 border-t border-white/10" /><WebsiteIntelligenceReportView report={report.reportSummary.canonicalWebsiteReport} /></section>}
