...
import { WebsiteChangeTimeline } from "../components/WebsiteChangeTimeline";
...
function ExecutiveBrief({ report }: { report: ShadowScoreReport }) {
  ...
  return (
    <article className="mt-8 rounded-[32px] border border-white/10 bg-black/55 p-6 shadow-[0_0_60px_rgba(120,0,20,0.16)] sm:p-8">
      ...
      {report.reportSummary?.canonicalWebsiteReport && <section className="mt-5 rounded-3xl border border-white/10 bg-black/35 p-6" aria-label="Website Intelligence"><WebsiteIntelligenceDashboard report={report.reportSummary.canonicalWebsiteReport} />{report.reportSummary.websiteAlertSummary && <p className="mt-6 rounded-2xl border border-white/10 p-4 text-sm text-zinc-300"><strong>{report.reportSummary.websiteAlertSummary.count} risk alerts</strong>{Object.keys(report.reportSummary.websiteAlertSummary.severities).length ? `: ${Object.entries(report.reportSummary.websiteAlertSummary.severities).map(([severity, count]) => `${count} ${severity}`).join(", ")}` : "."}</p>}<WebsiteChangeTimeline changeReport={report.reportSummary.websiteChangeReport} timeline={report.reportSummary.websiteChangeTimeline} /><div className="my-10 border-t border-white/10" /><WebsiteIntelligenceReportView report={report.reportSummary.canonicalWebsiteReport} /></section>}
