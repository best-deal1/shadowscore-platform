...
export default function ReportFlow({ reportId, mode }: { reportId: string; mode: Mode }) {
  ...
  const product = productForInvestigation(report?.scanMode === "identity" ? "personal_identity" : "business");
  ...
  return <section className="mx-auto w-full max-w-[90rem] text-slate-900">
    ...
    {!loading && report && mode === "unlock" && <div className="mx-auto mt-8 max-w-4xl">
      ...
      <div className="mb-8 text-center"><p className="text-xs font-bold uppercase tracking-[.22em] text-emerald-300">Review and confirm</p><h1 className="mt-3 text-4xl font-black">{product.confirmationTitle}</h1><p className="mt-3 text-zinc-400">{report.scanMode === "identity" ? <>One Personal Identity Investigation produces one Executive Report for a one-time price of {REPORT_PRODUCT.price}.</> : <>One Business Investigation produces one Executive Report for a one-time price of {REPORT_PRODUCT.price}.</>}</p></div>
      <div className="grid overflow-hidden rounded-[32px] border border-white/10 bg-white/[.035] shadow-2xl lg:grid-cols-[1fr_.72fr]">
      <section className="p-7 sm:p-9"><p className="text-xs font-bold uppercase tracking-[.18em] text-emerald-300">{product.recordLabel}</p><h2 className="mt-2 text-lg font-black">Executive Report includes</h2><ul className="mt-5 space-y-4 text-sm text-zinc-300">{REPORT_PRODUCT.includes.map((item) => <li className="flex gap-3" key={item}><span className="text-emerald-300">✓</span>{item}</li>)}</ul><dl className="mt-8 border-t border-white/10 pt-6 text-sm"><div className="flex justify-between gap-4"><dt className="text-zinc-500">{product.subjectLabel}</dt><dd className="font-bold text-right">{report.entity}</dd></div><div className="mt-3 flex justify-between gap-4"><dt className="text-zinc-500">Investigation reference</dt><dd className="font-mono">{report.intakeId || report.reportId}</dd></div><div className="mt-3 flex justify-between gap-4"><dt className="text-zinc-500">Scope</dt><dd className="font-bold">{report.scanMode || report.platform}</dd></div>{sourceCount !== undefined && <div className="mt-3 flex justify-between gap-4"><dt className="text-zinc-500">Sources checked</dt><dd className="font-bold">{sourceCount}</dd></div>}</dl><Link href="/intake?resume=checkout" className="mt-5 inline-block text-sm font-bold underline">Correct Business or scope</Link></section>
