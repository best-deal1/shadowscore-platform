...
function linkClass(active: boolean) {
  return `rounded-full px-3 py-2 transition focus:outline-none focus:ring-2 focus:ring-sky-300 ${active ? "bg-white/10 text-white" : "text-zinc-400 hover:text-white"}`;
}
    ...
    <div className="min-h-screen overflow-x-hidden bg-black text-white">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-black/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <Link href="/" aria-label="ShadowScore home" className="flex items-center gap-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-300">
            <img src="/shadowscore-shield-v8.png" alt="" className="h-9 w-9 rounded-xl bg-black object-contain p-1" />
            <div className="text-xl font-black tracking-tight">Shadow<span className="text-sky-400">Score</span></div>
          </Link>
          ...
          <div className="hidden items-center gap-3 lg:flex">
            ...
            <Link href="/intake" className="rounded-full bg-sky-600 px-5 py-2 text-sm font-black text-white shadow-lg shadow-sky-950/40 transition hover:bg-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-300">Start Investigation</Link>
          </div>

          <button type="button" aria-label="Open navigation menu" aria-expanded={menuOpen} aria-controls="mobile-navigation" onClick={() => setMenuOpen((open) => !open)} className="rounded-xl border border-white/10 px-3 py-2 text-sm font-black text-white focus:outline-none focus:ring-2 focus:ring-sky-300 lg:hidden">
            Menu
          </button>
        </div>
        {menuOpen ? (
          <nav id="mobile-navigation" aria-label="Mobile navigation" onClick={() => setMenuOpen(false)} className="border-t border-white/10 bg-black px-4 py-4 lg:hidden">
            <div className="grid gap-2">
              <Link href="/intake" className="rounded-2xl bg-sky-600 px-4 py-3 text-center text-sm font-black text-white">Start Investigation</Link>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 border-b border-white/10 pb-10 lg:grid-cols-[1.2fr_0.8fr]">
          <div>
            <div className="flex items-center gap-3">
              ...
              <div className="text-2xl font-black tracking-tight">Shadow<span className="text-sky-400">Score</span></div>
            </div>
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6">
            <div className="text-xs font-black uppercase tracking-[0.24em] text-sky-200">Ready to investigate?</div>
            <p className="mt-3 text-sm leading-6 text-zinc-400">Start with a website, company, seller, email or phone number.</p>
            <Link href="/intake" className="mt-5 inline-flex rounded-full bg-sky-600 px-5 py-3 text-xs font-black uppercase tracking-[0.14em] text-white hover:bg-sky-500">Start Investigation</Link>
          </div>
        </div>
        <div className="mx-auto mt-10 grid max-w-7xl gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {footerGroups.map((group) => (
            <nav key={group.title} aria-label={`${group.title} footer`} className="space-y-3">
              ...
              {group.links.map((link) => <Link key={link.href} href={link.href} className="block text-sm font-bold text-zinc-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-sky-300">{link.label}</Link>)}
            </nav>
          ))}
          <nav aria-label="Connect footer" className="space-y-3">
            ...
            {socialLinks.map((link) => <a key={link.label} href={link.href} target="_blank" rel="noreferrer" className="block text-sm font-bold text-zinc-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-sky-300">{link.label}</a>)}
          </nav>
        </div>
